...
进入队列的头部。当一个工作线程中没有任务时，会从其他工作线程的队列尾部获取一个任务。
...
public class ForkJoinPoolTask extends RecursiveTask<Integer> {
	...
	protected Integer compute() {
		...
		if (canCompute) {
			...
			ForkJoinPoolTask leftTask = new ForkJoinPoolTask(start, middle);
			ForkJoinPoolTask rightTask = new ForkJoinPoolTask(middle + 1, end);
	...
	public static void main(String[] args) {
		...
		// 生成一个计算任务，计算1+2+3+4
		ForkJoinPoolTask task = new ForkJoinPoolTask(1, 100);
