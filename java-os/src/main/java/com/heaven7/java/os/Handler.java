...
public class Handler {
	...
	public Handler(Callback callback, boolean async) {
		if (FIND_POTENTIAL_LEAKS) {
			...
			if ((klass.isAnonymousClass() || klass.isMemberClass() || klass.isLocalClass())
				...
				System.err.println("Handler: The following Handler class should be static or leaks might occur: "
	 ...
	 * will add an additional delay to execution. The runnable will be run on
	 ...
	 * <p>
	 * If the current thread is the same as the handler thread, then the
	 * runnable runs immediately without being enqueued. Otherwise, posts the
	 * runnable to the handler and waits for it to complete before returning.
	 * </p>
	 * <p>
	 * This method is dangerous! Improper use can result in deadlocks. Never
	 * call this method while any locks are held or use it in a possibly
	 * re-entrant manner.
	 * </p>
	 * <p>
	 * This method is occasionally useful in situations where a background
	 * thread must synchronously await completion of a task that must run on the
	 ...
	 * runnable will remain posted on the handler and may already be in progress
	 * or complete at a later time.
	 * </p>
	 * <p>
	 * When using this method, be sure to use {@link Looper#quitSafely} when
	 * quitting the looper. Otherwise {@link #runWithScissors} may hang
	 * indefinitely. (TODO: We should fix this by making MessageQueue aware of
	 * blocking runnables.)
	 * </p>
	 *
	 * @param r
	 ...
	 * <b>The time-base is {@link android.os.SystemClock#uptimeMillis}.</b> Time
	 * spent in deep sleep will add an additional delay to execution. You will
	 * receive it in {@link #handleMessage}, in the thread attached to this
	 * handler.
	 * 
	 * @param uptimeMillis
